...
const ParticipantList: React.FC<ParticipantListProps> = ({ participants, currentUserId }) => (
  <div style={{ padding: "16px", height: "100%", display: "flex", flexDirection: "column" }}>
    <h4 style={{ marginBottom: "4px", fontSize: "0.9rem", color: "var(--text-dim)", textTransform: "uppercase", letterSpacing: "1px" }}>Online ({participants.length})</h4>
    <div style={{ flex: 1, overflowY: "auto", marginTop: "16px", display: "flex", flexDirection: "column", gap: "8px" }}>
      {participants.length === 0 && <p style={{ color: "var(--text-dim)", fontSize: "0.85rem" }}>No one here yet</p>}
      {participants.map((p) => (
        <div key={p.id} style={{ display: "flex", alignItems: "center", gap: "10px", padding: "8px", borderRadius: "10px", background: p.user.id === currentUserId ? "rgba(139,92,246,0.15)" : "transparent", fontSize: "0.9rem" }}>
          ...
          <div style={{ minWidth: 0 }}>
            <div style={{ fontWeight: 500 }}>{p.user.username}{p.user.id === currentUserId ? <span style={{ fontSize: "0.7rem", color: "var(--text-dim)" }}> (You)</span> : ""}</div>
            <div style={{ fontSize: "0.7rem", color: "var(--text-dim)" }}>{p.role === "OWNER" ? "Host" : "Viewer"}</div>
          </div>
        </div>
      ))}
    </div>
